...
function FlightRow({ flight, data, onStatus, onDetails }) {
  ...
  return <div className="flight-row">
    ...
    <RouteElement className={`route ${trip?'flight-trip-link':''}`} onClick={trip?()=>onDetails(trip):undefined} aria-label={trip?`View ${trip.customer_name} trip details`:undefined}><div><b>{flight.origin}</b><span></span><Plane size={14}/><span></span><b>{flight.destination}</b></div><small>{flight.flight_number} · {aircraft?.model || 'Unassigned aircraft'}</small>{flight.distance_nm&&<small className="flight-estimate-line">{Math.round(flight.distance_nm)} NM · {durationFmt(flight.estimated_leg_time_minutes)} leg · {flight.estimated_fuel_usage_gallons == null ? '—' : `${Math.round(flight.estimated_fuel_usage_gallons)} gal`}</small>}{trip&&<Eye className="flight-trip-eye" size={14}/>}</RouteElement>
...
function TripDetailsModal({ trip, data, onClose }) {
  ...
  return <div className="modal-backdrop" onMouseDown={e=>e.target===e.currentTarget&&onClose()}><article className="modal trip-detail-modal" role="dialog" aria-modal="true" aria-labelledby="trip-detail-title">
    ...
    <div className="trip-detail-body">
      ...
      <section className="trip-detail-section full"><div className="trip-detail-section-title"><span><CalendarDays size={15}/></span><div><h3>Flight legs</h3><p>{flights.length?`${flights.length} linked ${flights.length===1?'leg':'legs'}`:'No flight legs scheduled'}</p></div></div>{flights.length?<div className="trip-detail-legs">{flights.map((flight,index)=>{const legAircraft=data.aircraft.find(a=>a.id===flight.aircraft_id);return <div key={flight.id}><em>{index+1}</em><div><b>{flight.origin} <ArrowRight size={13}/> {flight.destination}</b><small>{flight.flight_number} · {legAircraft?.tail_number||'No aircraft'}</small>{flight.distance_nm&&<small className="leg-estimate-summary">{Math.round(flight.distance_nm)} NM · {durationFmt(flight.estimated_flight_time_minutes)} in air · {durationFmt(flight.estimated_leg_time_minutes)} leg · {flight.estimated_fuel_usage_gallons == null?'—':`${Math.round(flight.estimated_fuel_usage_gallons)} gal fuel`}</small>}</div><div><b>{dateTimeFmt(flight.scheduled_departure)}</b><small>Arrives {dateTimeFmt(flight.scheduled_arrival)}</small></div><Badge status={flight.status}/></div>})}</div>:<div className="trip-detail-empty">This trip has not been added to the flight schedule.</div>}</section>

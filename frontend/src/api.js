...
export const api = {
  ...
  airports: (query, limit = 8) => request(`/airports?${new URLSearchParams({ query, limit })}`),
  airport: (code) => request(`/airports/${encodeURIComponent(code)}`),
  airportDistance: (origin, destination) => request(`/airports/distance?${new URLSearchParams({ origin, destination })}`),
